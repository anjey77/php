...
class ShippingTotal extends \Magento\Sales\Block\Adminhtml\Items\Column\DefaultColumn
{
    ...
    public function getShipping($item) {
        ...
        $shippingTotal = $this->getShippingRate($quoteId,$vendorId,$locationId);
        return $this->formatToBaseCurrency($shippingTotal);
    }

    public function getShippingRate($quoteId,$vendorId,$locationId) {
        $query = 'SELECT amount FROM omnyfy_vendor_quote_shipping WHERE quote_id = '.$quoteId.' && vendor_id = '.$vendorId.' && location_id ='.$locationId;
