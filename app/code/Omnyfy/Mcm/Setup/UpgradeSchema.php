...
class UpgradeSchema implements UpgradeSchemaInterface {
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context) {
        ...
        if (version_compare($version, '1.1.20') < 0) {
            /**
             * Create table 'omnyfy_mcm_payout_type'
             */
            $table = $installer->getConnection()->newTable(
                $installer->getTable('omnyfy_mcm_payout_type')
            )->addColumn(
                'id',
            ...
            $installer->getConnection()->createTable($table);
            /**
             * Create table 'omnyfy_mcm_vendor_payout_type'
             */
            $table = $installer->getConnection()->newTable(
                $installer->getTable('omnyfy_mcm_vendor_payout_type')
            )->addColumn(
                'id',
                \Magento\Framework\DB\Ddl\Table::TYPE_SMALLINT,
                null,
                ...
                $installer->getFkName('omnyfy_mcm_vendor_payout_type', 'vendor_id', 'omnyfy_vendor_vendor_entity', 'entity_id'),
                'vendor_id',
                $installer->getTable('omnyfy_vendor_vendor_entity'),
                'entity_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->addForeignKey(
                $installer->getFkName('omnyfy_mcm_vendor_payout_type', 'payout_type_id', 'omnyfy_mcm_payout_type', 'id'),
                'payout_type_id',
                $installer->getTable('omnyfy_mcm_payout_type'),
                'id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->setComment(
                'Vendor Payout Type table'
            );
            $installer->getConnection()->createTable($table);
        }

        if (version_compare($version, '1.1.21') < 0) {
            $connection->addColumn(
                $setup->getTable('omnyfy_mcm_vendor_payout_history'), 'payout_type_id', [
                    'type' => Table::TYPE_SMALLINT,
                    'nullable' => true,
                    'comment' => 'Payout Type Id'
                ]
            );
        }

        if (version_compare($version, '1.1.22') < 0) {
            $connection->addColumn(
                $setup->getTable('omnyfy_mcm_vendor_order'), 'refund_amount', [
                    'type' => Table::TYPE_DECIMAL,
                    'length' => '10,2',
                    'nullable' => true,
                    'default' => 0.00,
            ...
            $connection->addColumn(
                $setup->getTable('omnyfy_mcm_vendor_order'), 'base_refund_amount', [
                    'type' => Table::TYPE_DECIMAL,
                    'length' => '10,2',
                    'nullable' => true,
                    'default' => 0.00,
            ...
            $connection->addColumn(
                $setup->getTable('omnyfy_mcm_vendor_order'), 'refund_shipping_amount', [
                    'type' => Table::TYPE_DECIMAL,
                    'length' => '10,2',
                    'nullable' => true,
                    'default' => 0.00,
                    'comment' => 'Refund Shipping Amount'
                ]
            );
            $connection->addColumn(
                $setup->getTable('omnyfy_mcm_vendor_order'), 'base_refund_shipping_amount', [
                    'type' => Table::TYPE_DECIMAL,
                    'length' => '10,2',
                    'nullable' => true,
                    'default' => 0.00,
                    'comment' => 'Base Refund Shipping Amount'
                ]
            );
        }
