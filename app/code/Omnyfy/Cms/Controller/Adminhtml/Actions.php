...
abstract class Actions extends \Magento\Backend\App\Action {
    ...
    protected function _indexAction() {
        //forward to edit
        if ($this->getRequest()->getRouteName() == 'cms' && $this->getRequest()->getControllerName() == 'category') {
            $this->_redirect('*/*/edit');
        }
    ...
    public function _saveAction() {
        ...
        try {
            ...
            $this->_objectManager->get('Psr\Log\LoggerInterface')->critical('form data' . print_r($model->getData(), true));
        } catch (\Exception $e) {
            $this->_objectManager->get('Psr\Log\LoggerInterface')->critical('form data' . print_r($model->getData(), true));
    ...
    protected function _massStatusAction() {
        $ids = $this->getRequest()->getParam($this->_massActionRequestKey);
        if (!is_array($ids)) {
            $ids = [$ids];
        }
