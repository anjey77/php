 ...
 */
namespace Omnyfy\VendorReview\Block\Adminhtml\Rating\Edit\Tab;
...
class Form extends \Magento\Backend\Block\Widget\Form\Generic
{
     ...
     * @var string
     */
    protected $_template = 'Omnyfy_VendorReview::rating/form.phtml';
     ...
     * @param \Omnyfy\VendorReview\Model\Rating\OptionFactory $optionFactory
    ...
    public function __construct(
        ...
        \Omnyfy\VendorReview\Model\Rating\OptionFactory $optionFactory,
    ...
    protected function addRatingFieldset()
    {
        ...
        if($ratingData && $ratingData->getData('vendor_rating_code') == Rating::DEFAULT_RATING_CODE){
            $readOnly = true;
        }
        ...
        foreach ($this->systemStore->getStoreCollection() as $store) {
            $this->getFieldset('rating_form')->addField(
                'vendor_rating_code_' . $store->getId(),
                'text',
                ['label' => $store->getName(), 'name' => 'vendor_rating_codes[' . $store->getId() . ']']
            );
        }
        $this->setRatingData();
    }
    ...
    protected function setRatingData()
    {
        if ($this->session->getRatingData()) {
            ...
            if (isset($data['vendor_rating_codes'])) {
                $this->setRatingCodes($data['vendor_rating_codes']);
            }
            ...
            if ($this->_coreRegistry->registry('rating_data')->getVendorRatingCodes()) {
                $this->setRatingCodes($this->_coreRegistry->registry('rating_data')->getVendorRatingCodes());
            }
        }
    ...
    protected function setRatingCodes($ratingCodes)
    {
        foreach ($ratingCodes as $store => $value) {
            $element = $this->getForm()->getElement('vendor_rating_code_' . $store);
            if ($element) {
