 ...
 */
namespace Omnyfy\VendorReview\Model\ResourceModel;
...
class Rating extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
     ...
     * @param \Omnyfy\VendorReview\Model\ResourceModel\Review\Summary $reviewSummary
    ...
    public function __construct(
        ...
        \Omnyfy\VendorReview\Model\ResourceModel\Review\Summary $reviewSummary,
        $connectionName = null
    ) {
    ...
    {
        $this->_init('vendor_rating', 'vendor_rating_id');
    }
    ...
    {
        $this->_uniqueFields = [['field' => 'vendor_rating_code', 'title' => '']];
        return $this;
    }
     ...
     * @param string $field
     * @param mixed $value
     * @param \Omnyfy\VendorReview\Model\Rating $object
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        $codeExpr = $connection->getIfNullSql('title.value', "{$table}.vendor_rating_code");

        $select->joinLeft(
            ['title' => $this->getTable('vendor_rating_title')],
            $connection->quoteInto("{$table}.vendor_rating_id = title.vendor_rating_id AND title.store_id = ?", $storeId),
            ['vendor_rating_code' => $codeExpr]
        );
     ...
     * Actions after load
     *
     * @param \Magento\Framework\Model\AbstractModel|\Omnyfy\VendorReview\Model\Rating $object
     * @return $this
     */
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $connection = $this->getConnection();
        $bind = [':vendor_rating_id' => (int)$object->getId()];
        // load rating titles
        $select = $connection->select()->from(
            $this->getTable('vendor_rating_title'),
            ['store_id', 'value']
        )->where(
    ...
    public function getStores($ratingId)
    {
        $select = $this->getConnection()->select()->from(
            $this->getTable('vendor_rating_store'),
            'store_id'
        )->where(
            'vendor_rating_id = ?',
            $ratingId
    ...
    protected function processRatingCodes(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $table = $this->getTable('vendor_rating_title');
        $select = $connection->select()->from($table, ['store_id', 'value'])
            ->where('vendor_rating_id = :vendor_rating_id');
        $old = $connection->fetchPairs($select, [':vendor_rating_id' => $ratingId]);
        ...
        foreach (array_diff_assoc($new, $old) as $storeId => $title) {
            $insert[] = ['vendor_rating_id' => $ratingId, 'store_id' => (int)$storeId, 'value' => $title];
        }
    ...
    protected function deleteRatingData($ratingId, $table, array $storeIds)
    {
        ...
        try {
            $where = ['vendor_rating_id = ?' => $ratingId, 'store_id IN(?)' => $storeIds];
    ...
    protected function _afterDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if (!$this->moduleManager->isEnabled('Omnyfy_VendorReview')) {
            return $this;
        }
    ...
    protected function _getEntitySummaryData($object)
    {
        ...
        $select = $connection->select()->from(
            ['rating_vote' => $this->getTable('vendor_rating_option_vote')],
            ['entity_pk_value' => 'rating_vote.entity_pk_value', 'sum' => $sumColumn, 'count' => $countColumn]
        )->join(
            ['vendor_rating' => $this->getTable('vendor_rating')],
            'vendor_rating.vendor_rating_id = rating_vote.vendor_rating_id',
            []
        )->join(
            ['review' => $this->getTable('vendor_review')],
            'rating_vote.omnyfy_vendor_review_id=review.omnyfy_vendor_review_id',
            []
        )->joinLeft(
            ['review_store' => $this->getTable('omnyfy_vendor_review_store')],
            'rating_vote.omnyfy_vendor_review_id=review_store.omnyfy_vendor_review_id',
            ['review_store.store_id']
        )->where('vendor_rating.vendor_rating_code = ?', \Omnyfy\VendorReview\Model\Rating::DEFAULT_RATING_CODE);
        if (!$this->_storeManager->isSingleStoreMode()) {
            $select->join(
                ['rating_store' => $this->getTable('vendor_rating_store')],
                'rating_store.vendor_rating_id = rating_vote.vendor_rating_id AND rating_store.store_id = review_store.store_id',
                []
            );
        }
        $select->join(
            ['review_status' => $this->getTable('omnyfy_vendor_review_status')],
    ...
    public function getReviewSummary($object, $onlyForCurrentStore = true)
    {
        ...
        $select->where(
            'rating_vote.omnyfy_vendor_review_id = :omnyfy_vendor_review_id'
        )->group(
            'rating_vote.omnyfy_vendor_review_id'
        )->group(
            'review_store.store_id'
        );

        $data = $connection->fetchAll($select, [':omnyfy_vendor_review_id' => $object->getOmnyfyVendorReviewId()]);
    ...
    /**
     * Delete ratings by vendor id
     *
     * @param int $vendorId
     * @return $this
     */
    public function deleteAggregatedRatingsByVendorId($vendorId)
    {
        $entityId = $this->getEntityIdByCode(\Omnyfy\VendorReview\Model\Rating::ENTITY_PRODUCT_CODE);
        $connection = $this->getConnection();
        $select = $connection->select()->from($this->getMainTable(), 'vendor_rating_id')->where('entity_id = :entity_id');
        $ratingIds = $connection->fetchCol($select, [':entity_id' => $entityId]);

        if ($ratingIds) {
            $where = ['entity_pk_value = ?' => (int)$vendorId, 'vendor_rating_id IN(?)' => $ratingIds];
            $connection->delete($this->getTable('vendor_rating_option_vote_aggregated'), $where);
        }
