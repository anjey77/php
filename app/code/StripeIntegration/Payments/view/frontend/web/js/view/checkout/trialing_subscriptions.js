define(
    ...
    function (ko, Component, quote, priceUtils, totals, $t, getTrialingSubscriptions) {
        "use strict";
        return Component.extend({
            ...
            refresh: function(quote)
            {
                ...
                var subs = window.checkoutConfig.payment['stripe_payments'].trialingSubscriptions;
                if (!subs || !('subscriptions_total' in subs) || subs.subscriptions_total == 0)
                    return;
                ...
                getTrialingSubscriptions(quote)
                    ...
                    .done(function (subscriptions)
                    {
                        try {
                            ...
                            console.warn('Could not retrieve trialing subscriptions: ' + e.message);
                    ...
                    {
                        self.trialingSubscriptions(window.checkoutConfig.payment['stripe_payments'].trialingSubscriptions);
                        console.warn(console.warn('Could not retrieve trialing subscriptions: ' + err));
                    });
            },
