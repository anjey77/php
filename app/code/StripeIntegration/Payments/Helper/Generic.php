...
class Generic
{
    ...
    public function getTrialingSubscriptionsAmounts()
    {
        ...
        foreach ($items as $item)
        {
            ...
            $enabled = $product->getStripeSubEnabled();
            $trial = $product->getStripeSubTrial();
            if ($enabled && is_numeric($trial) && $trial > 0)
            {
    ...
    public function capture($token, $payment, $amount, $useSavedCard = false)
    {
        ...
        try
        {
            ...
            else if ($currency == strtolower($payment->getOrder()->getBaseOrderCurrencyCode()))
                $finalAmount = $amount;
            else
            ...
            else if ($ch->captured)
            {
                ...
                $this->dieWithError("This invoice has already been captured in Stripe for an amount of $humanReadableAmount. To complete the order, please re-create the Invoice using the Offline capture method.");
            }
            else // status == pending
            {
